...
class _Visitor(nodes.NodeVisitor):
    ...
    def dispatch_visit(self, node):
        ...
        elif cls is nodes.line_block:
            box = RstLineBlock()
            self.current.add_widget(box)
            self.push(box)
            self.idx_list = None
        elif cls is nodes.line:
            # self.do_strip_text = True
            label = RstLine(document=self.root)
            self.current.add_widget(label)
            self.push(label)
    ...
    def dispatch_departure(self, node):
        ...
        if cls is nodes.document:
            ...
            # self.do_strip_text = False
            assert(isinstance(self.current, RstLine))
            self.set_text(self.current, 'line')
            self.pop()
